<template>
  <div class="home">
    ...
    <div class="sudoku_row"  >
      <div class="sudoku_item " :class="{opacity:curSelect===item.course_id, recommend:item.is_recommend == '1', 'first_recommend':item.is_recommend == '1' && index == 0}"
           v-for="(item,index) in languagesInfo"
           :key="index" @touchstart="touchstart(item.course_id)" @touchend="touchend" @click="toPaper(item.course_id)">
        <img v-if="item.course_cover" :src="item.course_cover" width="40" height="40" />
        {{item.course_name}}
      </div>
    </div>
